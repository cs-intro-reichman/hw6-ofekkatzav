...
public class Runigram {
	...
	public static Color[][] read(String fileName) {
		...
		for (int i = 0; i < numRows; i++) {
			...
			for (int j = 0; j < numCols; j++) {
				Color currColor = new Color(in.readInt(),in.readInt(),in.readInt());
				image[i][j] = currColor;
			}
		}
		return image;
	...
	public static Color[][] flippedVertically(Color[][] image) {
		Color[][] result = new Color[image.length][image[0].length];
		int rowIndex = image.length - 1;
	
		for (int i = 0; i < result.length; i++) {
			for (int j = 0; j < result[0].length; j++) {
				result[i][j] = image[rowIndex][j]; 
			}
			rowIndex--;
		}
		return result;
	...
	public static Color[][] scaled(Color[][] image, int width, int height) {
		...
		for (int i = 0; i < height; i++) {
			for (int j = 0; j < width; j++) {
				int originalI = (int)(i * (image.length / (double)height));
				int originalJ = (int)(j * (image[0].length / (double)width));
				result[i][j] = image[originalI][originalJ];
			}
    }
    return result;
	}
	...
	public static Color blend(Color c1, Color c2, double alpha) {
		...
		Color result = new Color(blendedRed,blendedGreen,blendedBlue);
	...
	public static void morph(Color[][] source, Color[][] target, int n) {
		...
		Color[][] result = new Color[sourceWidth][sourceHeight];
		for (int i = 0; i < n; i++) {
